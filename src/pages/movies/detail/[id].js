          ...
          <div className={css.details}>
            ...
            <section className={css.summarySection}>
              ...
              <p className={css.overview}>
                {overview?.trim() ||
                  tagline?.trim() ||
                  `${title} hakkında özet bilgi mevcut değil.`}
              </p>
            </section>
...
export async function getServerSideProps({ params }) {
  ...
  const movie = await fetchDetailsById("movies", id);
  const credits = await fetchCreditsById("movies", id);
  const videos = await fetchVideosById("movies", id);
